    ...
    public void run() {
        ...
        switch (command) {
            case Protocol.DOWNLOAD:
                System.out.println(sourceIP.toString() + "/" + sourcePort + " | DOWNLOAD: " +  new String(data));
//                Reactions.sendDownloadApproved(host, data, sourceIP, sourcePort);
                break;

            case Protocol.UPLOAD:
                System.out.println(sourceIP.toString() + "/" + sourcePort + " | UPLOAD: " +  new String(data));
                Reactions.sendUploadApproved(host, data, sourceIP, sourcePort);
                break;

            case Protocol.UPLOAD_APPROVED:
                System.out.println(sourceIP.toString() + "/" + sourcePort + " | UPLOAD_APPROVED: " +  new String(data));
                Reactions.sendFile(host, data, sourceIP, sourcePort);
                break;

            case Protocol.ASKFILELIST:
                System.out.println(sourceIP.toString() + "/" + sourcePort + " | ASKFILELIST");
                Reactions.sendFileList(host, sourceIP, sourcePort);
                break;

            case Protocol.FILELIST:
                System.out.println(sourceIP.toString() + "/" + sourcePort + " | FILELIST");
            ...
            case Protocol.SENDDATA:
                ...
                System.out.println(sourceIP.toString() + "/" + sourcePort + " | SENDDATA: " + fileName + " " + nBytes + " bytes");
